...
static int				match_length = 0;  /* of longest match.  These are
										   set by the InsertNode() procedure. */
static int				lson[N + 1];
static int				rson[N + 257];
static int				dad[N + 1];  /* left & right children &
...
BOOL Lzss_Compress(BYTE *pInBuf, DWORD dwInBufSize, BYTE *pOutBuf, DWORD & dwOutSize)
{
	...
	for (len = 0; len < F && len < (INT)dwInBufSize; len++)
	{
		// Read F bytes into the last F bytes of the buffer 
		text_buf[r + len] = pInBuf[dwInPos++];  
	}
	for (i = 1; i <= F; i++) 
	{
	...
	do 
	{
		...
		{ 
			...
			for (i = 0; i < code_buf_ptr; i++)  /* Send at most 8 units of */
				pOutBuf[dwOutPos++] = code_buf[i];/* code together */

			code_buf[0] = 0;  
			code_buf_ptr = mask = 1;
		}
		last_match_length = match_length;
		for(i = 0; i<last_match_length && dwInPos<dwInBufSize; i++)
		{
			c = pInBuf[dwInPos++];
