...
retry:
	...
	CString cs;
	static TCHAR BASED_CODE szFilter[] = _T("CPack 包文件 (*.cpack) | *.cpack ||");
	CFileDialog f(TRUE, NULL, _T("*.cpack"), OFN_PATHMUSTEXIST | OFN_EXPLORER
	...
	if( nResult == IDOK )
	{
		...
		{
			if( lstrcmp((LPCTSTR)cs.Right(4), _T(".cpack")) != 0 )
			{
				cs += _T(".cpack");
			}
			this->SetTitle((LPCTSTR)cs);
